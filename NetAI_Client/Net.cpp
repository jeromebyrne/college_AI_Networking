...
Net::Net(void):m_lastPortNumberReceived(0)
{
}
...
bool Net::SetUpConnection(int port)
{
	...
	while (bind(m_sockfd, (struct sockaddr *)&m_my_addr, sizeof(m_my_addr)) == SOCKET_ERROR)
	{
		port++;
		//cout<<"Error! Trying next port !"<<portNum<<endl;			
		m_my_addr.sin_family = AF_INET; 
	...
	//specify how long you want to wait for data 
	m_tv.tv_sec = 0.00;
	m_tv.tv_usec = 0;
...
bool Net::ReceiveData(char * data)
{
		...
		if (ready)
		{
			...
			{	
				...
				m_lastPortNumberReceived = ntohs(m_their_addr.sin_port);
				if (WSAGetLastError() == 10054)
				{
					return false;
				}
				//string fromAdder = inet_ntoa(m_their_addr.sin_addr);
